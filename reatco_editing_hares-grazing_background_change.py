...
Model_DIR = 'tune_a_video_model/hares-grazing'
...
prompts = [
            "A cat and a swan are grazing in the grass, with the aurora borealis in the background.",
            "A cat and a swan are grazing in the yellow meadow."
          ]
maintain_back = False  # if conduct object editing, need to set Ture to maintain background
bbox = [[67, 240, 271, 353], [410, 150, 590, 343]]  # bbox for region of interest
token_indices_list = [
    [2, 5],
    [2, 5],
]  # align with bbox
...
if maintain_back:
    ...
    video_latents = torch.load('data/video_latents_hares-grazing.pt').to(torch.float16)
...
if use_inv_latent:
    ...
    with torch.inference_mode():
        ...
        ddim_inv_latents = ddim_inversion(
            pipe, ddim_inv_scheduler, video_latent=torch.load('data/video_latents_hares-grazing.pt').to(torch.float16),
