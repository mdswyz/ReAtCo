...
Model_DIR = 'tune_a_video_model/dolphins-swimming'
...
prompts = [
            "A jellyfish and a goldfish are swimming in the blue ocean.",
            "A goldfish and a shark are swimming in the blue ocean.",
            "A jellyfish and a octopus are swimming in the blue ocean.",
            "A jellyfish and a shark are swimming in the blue ocean.",
            "A turtle and a goldfish are swimming in the blue ocean.",
          ]
maintain_back = True  # if conduct object editing, need to set Ture to maintain background
bbox = [[93, 287, 269, 417], [262, 257, 477, 378]]  # bbox for region of interest
token_indices_list = [
    [2, 5],
    [2, 5],
    [2, 5],
...
video_length = 16
window_size = 4  # split short video clip frames for resource-friendly editing
height = 448
width = 768
...
if maintain_back:
    ...
    video_latents = torch.load('data/video_latents_dolphins-swimming.pt').to(torch.float16)
...
if use_inv_latent:
    ...
    with torch.inference_mode():
        ...
        ddim_inv_latents = ddim_inversion(
            pipe, ddim_inv_scheduler, video_latent=torch.load('data/video_latents_dolphins-swimming.pt').to(torch.float16),
